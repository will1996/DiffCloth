...
Simulation* makeSim(std::string exampleName, bool runBackward = true) {
  ...
  if(exampleName == "image_loss")
  {
    Simulation::SceneConfiguration initSceneProfile = OptimizationTaskConfigurations::ImageLossScene;
    sim = Simulation::createSystem(
                                   initSceneProfile,
                                   Vec3d(0, 0, 0), runBackward);
    std::cerr << "Initialized:  " << exampleName << std::endl;
...
PYBIND11_MODULE(diffcloth_py, m) {
  ...
  py::class_<Simulation::BackwardInformation>(m, "BackwardInformation")
          ...
          .def_readonly("dx_dk_pertype", &Simulation::BackwardInformation::dx_dk_pertype)
  ...
  py::class_<Simulation>(m, "Simulation")
          ...
          .def("ParameterEstimationForward", &Simulation::ParameterEstimationForward, "Take parameters, generate x at step nsteps", py::arg("nsteps"), py::arg("params"))
          .def("ParameterEstimationBackward",&Simulation::ParameterEstimationBackward, "GetParameterGradient", py::arg("ParamGradOut"))
          ...
          .def("GetMeshTriangles", &Simulation::GetMeshTriangles,
               "returns indices as an array of vec3")
