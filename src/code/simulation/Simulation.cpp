...
std::cerr<<"CHECKING FOR COLLISIONS"<<std::endl;
...
#pragma omp critical
    ...
    if (selfcollisionEnabled) { 
	    std::cerr<<"CHECKING FOR SELF COLLISIONS"<<std::endl;
...
std::pair<std::pair<SpMat, SpMat>, VecXd> Simulation::calculatedr_df(const completeCollisionInfo &infos,
  ...
  if (contactEnabled) {
    ...
    if (selfcollisionEnabled) {
      printf("CHECKING FOR SELF COLLISIONS");
      int layerCount = 0;
...
std::vector<Vec3i> Simulation::GetMeshTriangles()
{
  std::vector<Vec3i> trianglesOut;
  trianglesOut.reserve(mesh.size());
  for(Triangle& triangle : mesh){
    trianglesOut.push_back(triangle.getIdxVector());
  }
  return trianglesOut;
}
...
VecXd Simulation::ParameterEstimationForward(int nsteps, VecXd& params)
{
  std::cerr<<"Setting parameters to given parameter vec"<<std::endl;
  //Set triangle bending to parameter 0
  resetSystem(params[0]);
  std::cerr<<"updating x"<<std::endl;
  for(int i=0; i < nsteps; i++)
  {
    step() ;
  }
  return forwardRecords.back().x;
}

void Simulation::ParameterEstimationBackward(VecXd& paramGradOut)
{
}
...
#pragma omp parallel for if (OPENMP_ENABLED)
  ...
  for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++) {
    ...
    if (taskInfo.dL_dk_pertype[i]) {
        ...
        //Compute dx_dk[i] as M^(-1) * df_dk*h^2
        ret.dx_dk_pertype[i] = Area_inv*(1.0/sceneConfig.fabric.density) * df_dk * sceneConfig.timeStep*sceneConfig.timeStep;
...
void Simulation::initializePrefactoredMatrices() {
  ...
  std::printf("anal glands");
  std::printf(
          "precompute matrices + prefactorization... density is: %.3f SceneConfigtimestep is :1/%d timeStepIs:%.3f : stiffness is:(spring:%.2f,attachment:%.2f,stretching:%.2f,bending:%.2f)\n",
  ...
  for (int sysMatId = 0; sysMatId < sysMat.size(); sysMatId++) {
    ...
    for (int i = 0; i < Constraint::CONSTRAINT_NUM; i++) {
      ...
      std::printf("GOT HERE 2!\n");
    for (int i = 0; i < sysMat[sysMatId].constraints.size(); i++) {
      Constraint *s = sysMat[sysMatId].constraints[i];
      if(s->constraintType >= Constraint::CONSTRAINT_NUM){
	      std::cerr<<"INVALID CONSTARINT TYPE, ABOUT TO SEG FAULT"<<std::endl;
      ...
      std::printf("\n");
      std::printf("DONE PREFACTORIZING MATRICES!\n");
...
Simulation::isSelfCollision(const Triangle &a, const Particle &p3, const Vec3d v_a, const Vec3d v_b) const {
  ...
  info.collides = x1.norm()<COLLISION_EPSILON || x2.norm()<COLLISION_EPSILON || x3.norm() < COLLISION_EPSILON;
  return info;
}
