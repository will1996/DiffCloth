...
public:
    ...
    typedef std::pair<std::vector<PrimitiveCollisionInformation>, std::vector<SelfCollisionInformation>> collisionInfoPair;
    typedef std::pair<collisionInfoPair, std::vector<std::vector<SelfCollisionInformation>>> completeCollisionInfo;
    ...
    struct BackwardInformation {
        ...
        VecXd dx_dk_pertype[Constraint::CONSTRAINT_NUM];
    ...
    struct TaskSolveStatistics {
        ...
        std::vector<std::pair<ParamInfo, ForwardInformation>> completeForwardLog; // For each simulation, only log the last frame of forward sim
        std::vector<std::pair<ParamInfo, BackwardInformation>> completeBackwardLog; // For each backprop, only log the first frame of backprop
    ...
    struct CorresPondenceTargetInfo {
        ...
        CorresPondenceTargetInfo(int idx, Vec3d pos, std::vector<int> particleIdx) : frameIdx(idx), targetPos(pos),
                                                                                     particleIndices(particleIdx) {
        }
    };
...
//    static AttachmentConfigs restPosConfiguration;
    ...
    static volatile bool gravityEnabled, windEnabled, staticEnabled, contactEnabled, selfcollisionEnabled, enableConstantForcefield;
    ...
    Vec3d restShapeMinDim, restShapeMaxDim, restShapeMidPoint, meshInitTransofrmMinDim;
    ...
    struct SystemMatrix { // All these matrices are constraint weight dependent, so if constraint weight change, these matrices change
        // Additionally, in our simulation sometimes the constraints need to be changed because of change of attachment constraints
        SpMat P;
        SpMat A;
        SpMat A_t;
        SpMat C;
    ...
    std::vector<ForwardInformation> forwardRecords, groundTruthForwardRecords, linesearchRecords;
    std::vector<std::pair<std::vector<ForwardInformation>, std::vector<BackwardInformation>>> backwardOptimizationRecords;
    Simulation::BackwardTaskInformation taskInfo;
    std::vector<std::pair<Simulation::ParamInfo, double>> backwardOptimizationGuesses;
    ...
    void updateBackwardDefaultInfo() {
      ...
      backwardInfoDefault.dL_dxfixed_accum.resize(sysMat[0].fixedPoints.size() * 3);
      ...
      for (int sysMatId = 0; sysMatId < sysMat.size(); sysMatId++) {
        backwardInfoDefault.dL_dsplines[sysMatId].resize(sysMat[sysMatId].controlPointSplines.size());
        for (int i = 0; i < sysMat[sysMatId].controlPointSplines.size(); i++) {
          backwardInfoDefault.dL_dsplines[sysMatId][i] = VecXd(
                  sysMat[sysMatId].controlPointSplines[i].getParameterNumber());
    ...
    void setWindAncCollision(bool windEnable, bool collisionEnable, bool selfCollisionEnable,
    ...
    static glm::vec3 getLookAtPos(Simulation *system, Simulation::SceneConfiguration &profile) {
      ...
      switch (profile.camFocusPointType) {
        ...
        case CameraFocusPointType::POINT: {
    ...
    BackwardInformation
    stepBackwardNN(Simulation::BackwardTaskInformation &taskInfo, VecXd &dL_dxnew, VecXd &dL_dvnew,
    ...
    void resetForwardRecordsFromFolder(std::string subFolder) {
        std::vector<Vec3d> modelPoints;
      std::vector<Vec3i> modelTris;
      std::string fullFolderPath = std::string(SOURCE_PATH) + "/output/" + subFolder + "/";
      ...
      Logging::logNormal("Loaded " + std::to_string(frameFileNames.size()) + " cloth frame files from folder " + fullFolderPath + "\n");
      ...
      for (int i = 0; i < frameFileNames.size(); i++) {
        ...
        for (Primitive *p :  primitives) {
          ...
          if (p->isPrimitiveCollection) { // TODO: this is a hacky way to reset primitive trees with <= 2 levels; should be changed to recursion 
    ...
    void createClothMeshFromModel(std::vector<Vec3d> &particleIn, std::vector<Vec3i> &meshIn);
    ...
    void rotatePointsAccordingToConfig(std::vector<Vec3d> &particleIn) {
      switch (sceneConfig.orientation) {
        case Orientation::CUSTOM_ORIENTATION: {
          ...
          particleIn = rotatePointsAroundCenter(particleIn, axisToRotation(sceneConfig.upVector, Vec3d(0, 1, 0)));
          break;
        }
        ...
        case Orientation::DOWN: {
          ...
          particleIn = rotatePointsAroundCenter(particleIn, axisToRotation(Vec3d(0, 1, 0), Vec3d(0, 0, 1)));
          break;
        }

        case Orientation::BACK: {
          std::printf("orientation down: rotate\n");
          particleIn = rotatePointsAroundCenter(particleIn, axisToRotation(Vec3d(0, 0, 1), Vec3d(1, 0, 0)) *
                                                            axisToRotation(Vec3d(1, 0, 0), Vec3d(0, 0, -1)));
          break;
        }
    ...
    std::vector<Vec3d> rotatePointsAroundCenter(std::vector<Vec3d> &particleIn, Rotation rotation);

    std::vector<Vec3i> GetMeshTriangles();

    void
    loadWindSim2RealAnimationSequence(std::string folderName, std::vector<std::string> files, bool isRyanWhite = true);

    void initScene();


    SelfCollisionInformation
    isSelfCollision(const Particle &a, const Particle &b, const Vec3d &x_a, const Vec3d &x_b, const Vec3d &v_a,
                    const Vec3d &v_b) const;

    Simulation::SelfCollisionInformation
    isSelfCollision(const Triangle &a, const Particle &p3, const Vec3d v_a, const Vec3d v_b) const;


    PrimitiveCollisionInformation isInContactWithObstacle(const Vec3d &pos, const Vec3d &v_in, const VecXd &x_prim_new,
    ...
    //Goes from parameters to X vector storing all vertices
    VecXd ParameterEstimationForward(int nsteps, VecXd& params);
    //generates 
    void ParameterEstimationBackward(VecXd& paramGradOut);
    ...
    void writeMatrix(std::ofstream &myfile, MatXd &A, std::string name, int precision) {
      myfile << name.c_str() << "\n";
    ...
    void writeMatrix(std::ofstream &myfile, SpMat &A, std::string name, int precision) {
    ...
    void writeMatrixSparse(std::ofstream &myfile, SpMat &A, std::string name, int precision) {
      MatXd Adense = A.toDense();
      myfile << name.c_str() << "\n";
      myfile << A.nonZeros() << "\n";
      myfile << A.rows() << " " << A.cols() << "\n";
    ...
    void writeMatrix(std::ofstream &myfile, VecXd &delta_P, std::string name, int precision) {
      myfile << name.c_str() << "\n";
    ...
    static VecXd getParticleNormals(std::vector<Triangle> mesh, const VecXd &x_now);


    double calculateLossAndGradient(LossType &lossType, LossInfo &lossInfo, VecXd &dL_dx, VecXd &dL_dv, int idx,
                                    bool calculateLoss);

    void resetSystemWithParams(BackwardTaskInformation &taskConfiguration, ParamInfo &param);

    void calculateFiniteDiffLossArr(ParamInfo paramPlus, ParamInfo paramMinus, double delta,
                                    std::vector<ForwardInformation> &centerForward, std::vector<double> &L_arr,
                                    BackwardTaskInformation taskConfiguration, LossType lossType, LossInfo &lossInfo,
                                    int FORWARD_STEPS,
                                    const std::function<void(const std::string &value)> &setTextBoxCB = [](
                                            const std::string &v) {});;

    std::vector<BackwardInformation>
    finiteDifferenceBackward(BackwardTaskInformation taskConfiguration, LossType lossType, LossInfo &lossInfo,
                             int FORWARD_STEPS, ParamInfo guess,
                             const std::function<void(const std::string &value)> &setTextBoxCB = [](
                                     const std::string &v) {});


    static std::string
    forwardInfoToString(BackwardTaskInformation taskInfo, const Simulation::ForwardInformation &forwardInfo) {
      std::string out;
      out += "============Forward Info:======================\n";
      out += "Total PD Iters:" + std::to_string(forwardInfo.cumulateIter) + "\n";
      out += "Total Frames Converged:" + std::to_string(forwardInfo.totalConverged) + "\n";
      out += "Forward Total Runtime[ms]:" + d2str((double) (forwardInfo.totalRuntime * 1.0 / 1000.0), 5) + "\n";
      ...
      for (const TimerEntry &breakdown: forwardInfo.accumTimer) {
        ...
        out += label + "[ms]:" + d2str(duration / 1000.0, 5) + "\t\t|" + d2str(double(percentage * 100), 3) + "%\n";
      }

      return out;
    }

    static std::string backwrdInfoAndGradToString(const Simulation::BackwardTaskInformation &taskInfo, BackwardInformation grad);

    static std::string parameterToString(const Simulation::BackwardTaskInformation &taskInfo, ParamInfo paramGuess);
    ...
    static std::string taskInfoToString(const Simulation::BackwardTaskInformation &taskInfo);


    void exportStatistics(Demos demoIdx, TaskSolveStatistics &statistics, BackwardTaskInformation taskInfo,
                          bool writePerf = false);


    void exportOptimizationRecords(Demos demoIdx, std::string experimentNameGiven = "", bool useGiven = false);


    void exportConfig(int demoIdx, SceneConfiguration &config, std::string fileName);


    void exportFrameInfo(Simulation *system, Simulation::ForwardInformation &record, std::string fileName) {
      std::ofstream myfile;
      myfile.open(fileName);


      for (int fixedPointId = 0; fixedPointId < record.x_fixedpoints.rows() / 3; fixedPointId++) {
        Vec3d clipPos = record.x_fixedpoints.segment(fixedPointId * 3, 3);
        myfile << "CLIP_" << fixedPointId << ":" << d2str(clipPos[0], 5) << "," << d2str(clipPos[1], 5) << ","
               << d2str(clipPos[2], 5) << "\n";
    ...
    void exportStatsSimulations(std::vector<std::pair<ParamInfo, ForwardInformation>> &records, std::string subFolder,
                                int prevWrriten, int newWritten);

    void exportSimulation(std::string fileName, std::vector<ForwardInformation> &records,
    ...
    std::vector<BackwardInformation>
    runBackwardTask(BackwardTaskInformation taskConfiguration, LossType lossType, LossInfo &lossInfo,
                    TaskSolveStatistics &taskStatistics,
                    int FORWARD_STEPS, ParamInfo guess,
                    bool lossOnly,
                    const std::function<void(const std::string &value)> &setTextBoxCB = [](const std::string &v) {},
    ...
    std::pair<collisionInfoPair, std::vector<std::vector<SelfCollisionInformation>>>
    collisionDetection(const VecXd &x_n, const VecXd &v, const VecXd &x_prim, const VecXd &v_prim);

    std::vector<std::vector<Simulation::SelfCollisionInformation>>
    contactSorting(Simulation::collisionInfoPair &detections, std::map<int, std::set<int>> &selfCollisionMap,
                   MatXi &selfCollisionTable);

    std::pair<VecXd, VecXd>
    calculateDryFrictionVector(const VecXd &f,
    ...
    calculatedr_df(const completeCollisionInfo &infos, bool calculatePrimitiveGradient,
    ...
    std::vector<VecXd>
    calculatedr_dmu(const std::vector<PrimitiveCollisionInformation> &infos, const std::vector<int> &primIds) const;

//    VecXd calculateDryFrictionAndVelocityVector(const VecXd &f, VecXd &v) const;
    Vec3d calcualteDryFrictionForce(const Vec3d &n, const Vec3d &f_i, double mu, CollisionType &type) const;
...
private:
    ...
    const double fillForces(Eigen::VectorXd &f_int, Eigen::VectorXd &f_ext, const Eigen::VectorXd &velocityVec,
                            const Eigen::VectorXd &posVec, double t_now);

    static SpMat
    factorizeDirectSolverLLT(const SpMat &A, Eigen::SimplicialLLT<SpMat> &lltSolver, const std::string &warning_msg);

    SpMat
    factorizeDirectSolverSparseLU(const SpMat &A, Eigen::SparseLU<SpMat> &lltSolver, const std::string &warning_msg);
...
public:
    ...
    int getActionDim() {
      return sysMat[currentSysmatId].fixedPoints.size() * 3;
    }
    ...
    static std::string boolToString(std::string labelTrue, std::string labelFalse, bool v) {
      if (v)
        return labelTrue;
      return labelFalse;
    }
    ...
    // set printVerbose
    void setPrintVerbose(bool flag) {
      printVerbose = flag;
    }
    ...
    VecXd
    solveDirect(VecXd &dL_dxnew, double t_2, SpMat &dproj_dxnew_t, SystemMatrix &currentSysMat, SpMat &dr_df_plusI_t,
                SpMat &dr_df_t);
};
